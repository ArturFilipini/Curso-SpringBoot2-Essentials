...
class AnimeControllerIT {
    ...
    private static final DevDojoUser USER = DevDojoUser.builder().name("DevDojo Academy")
            .password("{bcrypt}$2a$10$aRQLBMTN9PZ0CNAnbsu40eNMsnC48kc0XQ2e/DIcTraSHJTDwef/6")
            .username("devdojo")
            .authorities("ROLE_USER")
            .build();
    private static final DevDojoUser ADMIN = DevDojoUser.builder().name("Artur Filipini")
            .password("{bcrypt}$2a$10$aRQLBMTN9PZ0CNAnbsu40eNMsnC48kc0XQ2e/DIcTraSHJTDwef/6")
            .username("artur")
    ...
    public TestRestTemplate testRestTemplateRoleAdminCreator(@Value("${local.server.port}") int port) {
        RestTemplateBuilder restTemplateBuilder = new RestTemplateBuilder()
                .rootUri("http://localhost:"+port)
                .basicAuthentication("artur", "academy");
        return new TestRestTemplate(restTemplateBuilder);
    }
}
    ...
    void delete_Returns403_WhenUserIsNotAdmin(){
        Anime savedAnime = animeRepository.save(AnimeCreator.createAnimeToBeSaved());
        //devDojoUserRepository.save(USER); tem que ficar abaixo do savedAnime para não ocorrer erro no test
        devDojoUserRepository.save(USER);
